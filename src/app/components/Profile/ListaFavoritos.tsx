...
import AccommodationCard from "../general/cards-arrendador/card"; // Ajusta el path si es necesario
...
export default function FavoritosList() {
  ...
  useEffect(() => {
    const fetchFavoritos = async () => {
      ...
      try {
        ...
        const ids = data.map((fav: any) => fav.propiedadId || fav.id || fav.propiedad?._id);
        setFavoritos(ids);
      } catch (err: any) {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 p-4">
      {favoritos.map((id) => (
        <AccommodationCard key={id} id={id} />
      ))}
    </div>
  );
}
